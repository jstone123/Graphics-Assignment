...
Texture2D BlurTexture;   // Texture containing the scene to copy to the full screen quad
...
float4 PPShockShader(PS_POSTPROCESS_INPUT ppIn) : SV_Target
{

	float2 uV = ppIn.UVScene;
	uV.y += cos(uV.x * 4 * 2 * 3.14159 + Wiggle) / 100;
	uV.x += cos(uV.y * 4 * 2 * 3.14159 + Wiggle) / 50;
	//sin(texcoord.y * 4 * 2 * 3.14159 + offset) / 100;
	float3 ppColour = SceneTexture.Sample(PointClamp, uV);//  * float3(0.0f,0.5f,0.5f);

	/*float3 temp = SceneTexture.Sample(PointClamp, ppIn.UVScene.x + sin(Wiggle));
	float3 temp1 = SceneTexture.Sample(PointClamp, ppIn.UVScene.y + sin(Wiggle));
...
float4 PPContrastShader(PS_POSTPROCESS_INPUT ppIn) : SV_Target
{
	// Sample the texture colour (look at shader above) and multiply it with the tint colour (variables near top)
	float3 ppColour = SceneTexture.Sample(PointClamp, ppIn.UVScene);


	ppColour = ((ppColour - 0.5f) * max(3,0)) + 0.5f;
